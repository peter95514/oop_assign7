...
void View::render(){
    ...
    for (int r = 0; r < GAME_WINDOW_HEIGHT; ++r) {
        ...
        for (int c = 0; c < GAME_WINDOW_WIDTH; ++c) {
            std::string& txt = latest_map[r][c];
            auto         fg  = latest_fg_color[r][c];
            auto         bg  = latest_bg_color[r][c];
            if(txt == " " && latest_vis[r][c] == 1) {
                txt = "█";
                fg = Color::WHITE;
                bg = Color::WHITE;
            }
...
void View::resetLatest(){
    for (int r = 0; r < GAME_WINDOW_HEIGHT; ++r) {
        std::fill(latest_vis[r].begin(), latest_vis[r].end(), false);
...
void View::changevis(std::vector<std::vector<bool>> input) {
    latest_vis = input;
}
